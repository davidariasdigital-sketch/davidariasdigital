...
serve(async (req) => {
  ...
  try {
    const { messages, quotationContext } = await req.json();
...
${quotationContext ? `\nContexto de la cotización actual:\n${quotationContext}` : ""}`;
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: "Error del asistente AI" }), {
    ...
    console.error("quotation-assistant error:", e);
