      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="liquid-glass rounded-[var(--radius)] p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })} className="bg-muted/50 border border-border rounded-xl px-4 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50">
        ...
        {pending.map((t) => (
          <div key={t.id} className="liquid-glass rounded-[var(--radius)] p-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {t.due_date && <p className="text-xs text-muted-foreground">{new Date(t.due_date).toLocaleDateString()}</p>}
            </div>
            <span className={`text-[10px] font-semibold px-2.5 py-0.5 rounded-full ${priorityColors[t.priority]}`}>{t.priority}</span>
