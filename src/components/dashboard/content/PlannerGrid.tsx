...
const ObjectiveColorPicker = ({ itemId }: { itemId: string }) => {
  ...
  return (
    <button
      ...
      className="flex-shrink-0 h-4 w-4 rounded-full border-2 shadow-sm hover:scale-110 transition-transform"
      style={{
        backgroundColor: color ?? "transparent",
        borderColor: color ? "#fff" : "hsl(0,0%,80%)",
        borderStyle: color ? "solid" : "dashed",
      }}
      title={current ? `Objetivo: ${current.label} (click para cambiar)` : "Click para asignar objetivo"}
    />
  );
};
